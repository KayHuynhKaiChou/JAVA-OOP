/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package IOTools;
import java.util.*;
/**
 *
 * @author DELL
 */
public class IOkeyboard {
    static final Scanner sc = new Scanner(System.in);
    //public static int numOfError = 0;
    
    public static String forcedMatchesRegex(String message,String error, String regrex) {
        boolean a = true;
        String value = null;
        while (a) {
            value = inputString(message);
//            if(value.isBlank()) value = "=";
            a = !value.matches(regrex);
            if(a) {System.out.println(error);}
        }
        return value;
    }
    
    public static int inputInteger(String inputND, String errorND, String regex) {
        //System.out.print(inputND);
        String input = forcedMatchesRegex(inputND,errorND, regex);
        return input.isEmpty() ? 0 : Integer.parseInt(input);
    }
    
    // ND : nội dung
    public static double inputDouble(String inputND, String errorND, String regex) {
        //System.out.print(inputND);
        String input = forcedMatchesRegex(inputND,errorND, regex);
        return input.isEmpty() ? 0 : Double.parseDouble(input);
    }
    
    public static String inputString(String inputND) {
        System.out.print(inputND);
        String str = sc.nextLine().trim();
        return str;
    }
    
//    public static String inputNoneEmptyString(String inputND, String errorAboutEmpty) {
//        while (true) {
//            String str = inputString(inputND);
//            if (str.length() == 0 || str.isEmpty()) {
//                System.err.println(errorAboutEmpty);
//            }else return str;
//        }
//    }
    
    
     public static boolean confirm(String inputND, String errorND) {
        String input;
        do {
            input = inputString(inputND + " (Y/N): ");
            if (!input.equals("Y") && !input.equals("y") && !input.equals("N") && !input.equals("n")) {
                System.out.println(errorND);
            }
        } while (!input.equals("Y") && !input.equals("y") && !input.equals("N") && !input.equals("n"));
        return (input.equals("Y") || input.equals("y"));
    }
     
    
}
